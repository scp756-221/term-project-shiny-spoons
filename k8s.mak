...
$(LOG_DIR)/dynamodb-init.log: cluster/cloudformationdynamodb.json
	...
	$(AWS) cloudformation create-stack --stack-name db-gonick --template-body file://$< || true | tee $(LOG_DIR)/dynamodb-init.log
...
#
dynamodb-clean:
	$(AWS) cloudformation delete-stack --stack-name db-gonick || true | tee $(LOG_DIR)/dynamodb-clean.log
...
$(LOG_DIR)/s3-$(S3_VER).repo.log: s3/Dockerfile s3/app.py s3/requirements.txt
	make -f k8s.mak --no-print-directory registry-login
	$(DK) build $(ARCH) -t $(CREG)/$(REGID)/cmpt756s3:$(S3_VER) s3 | tee $(LOG_DIR)/s3-$(S3_VER).img.log
...
cr: registry-login
	...
	$(DK) push $(CREG)/$(REGID)/cmpt756s3:$(S3_VER) | tee $(LOG_DIR)/s3repo.log
	$(DK) push $(CREG)/$(REGID)/cmpt756db:$(APP_VER_TAG) | tee $(LOG_DIR)/db.repo.log
